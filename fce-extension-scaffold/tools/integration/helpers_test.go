...
func deployFreshInstructionSender(t *testing.T) (common.Address, *helloworld.IncidentTriageInstructionSender) {
	t.Helper()
	time.Sleep(rpcDelay)
...
func deployInstructionSenderRaw(t *testing.T, registryAddr, machineRegistryAddr common.Address) error {
	...
	_, _, _, err = helloworld.DeployIncidentTriageInstructionSender(
		opts, testSupport.ChainClient,
