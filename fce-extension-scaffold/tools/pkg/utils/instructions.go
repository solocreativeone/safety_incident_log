...
func DeployInstructionSender(s *support.Support) (common.Address, *helloworld.IncidentTriageInstructionSender, error) {
	...
	address, tx, contract, err := helloworld.DeployIncidentTriageInstructionSender(
...
func SetExtensionId(s *support.Support, instructionSenderAddress common.Address) error {
	sender, err := helloworld.NewIncidentTriageInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	if err != nil {
		...
		if reason == "" {
			parsed, _ := helloworld.IncidentTriageInstructionSenderMetaData.GetAbi()
			if parsed != nil {
...
func SendClassifySeverity(s *support.Support, instructionSenderAddress common.Address, reportText string) (common.Hash, common.Hash, error) {
	sender, err := helloworld.NewIncidentTriageInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	tx, err := sender.SendClassifySeverity(opts, reportText)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := helloworld.IncidentTriageInstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("sendClassifySeverity", reportText)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
